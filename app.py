...
# --- Page Config ---
st.set_page_config(page_title="Flood Risk Prediction App", layout="wide")
st.title("🌧️ Flood Risk Prediction Dashboard")

# --- File Upload ---
st.sidebar.header("Upload CSV Data")
uploaded_file = st.sidebar.file_uploader("Upload your CSV with columns: state, district, latitude, longitude, date, rainfall_mm, river_level_mm")
...
if uploaded_file:
    ...
    # --- Risk Calculation (simple heuristic initially) ---
    # We'll assign risk based on rainfall and river level thresholds
    ...
    st.subheader("Data with Flood Risk")
    st.dataframe(df.head())
    ...
    sns.scatterplot(data=df, x='rainfall_mm', y='river_level_mm', hue='risk', palette={'Low':'green','Medium':'orange','High':'red'}, ax=ax2)
    ax2.set_title("Rainfall vs River Level")
    st.pyplot(fig2)

    # --- ML Model: Predict Risk from Rainfall and River Level ---
    st.subheader("Flood Risk Prediction Model")
